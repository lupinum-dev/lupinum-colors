...
import { formatCssVariables } from './export.js'
import {
  SHADE_NAMES,
  type Gamut,
  type OutputFormat,
...
function printHelp(): void {
  console.log(`Lupinum Colors — Tailwind shade generator

Usage:
  lupinum-colors <name> <color> [options]
...
Examples:
  lupinum-colors primary "#89E5D2" --seed exact --at auto --explain
  lupinum-colors primary "#89E5D2" --seed canonical --at 500 --variants
  lupinum-colors brand "oklch(62% 0.19 245)" --format tailwind`)
}
...
export function runCli(options: CliOptions): PaletteResult[] {
  ...
  const results = options.variants ? generateVariants(request) : [generatePalette(request)]
...
function printResult(result: PaletteResult, format: OutputFormat, inspect: boolean): void {
  if (format === 'css' || format === 'tailwind') {
    console.log(`/* hue path: ${result.configuration.huePath} */`)
    console.log(
      formatCssVariables(
        result.name,
        SHADE_NAMES.map((shade) => ({ shade, css: result.shades[shade].css })),
        format,
      ),
    )
    return
...
function printExplanation(result: PaletteResult): void {
  ...
  console.log(
    `Parsed: ${result.input.oklch.l.toFixed(4)} L, ${result.input.oklch.c.toFixed(4)} C, ${result.input.oklch.h.toFixed(2)}° H`,
  )
  console.log(
    `Reference: Tailwind ${result.reference.tailwindVersion}, ${result.reference.kind}, ${result.reference.model}`,
  )
  console.log(`Hue neighbors: ${result.reference.neighbors.join(' → ')}`)
  if (confidence !== undefined) {
    console.log(
      `Natural anchor: ${result.configuration.anchor} (${(confidence * 100).toFixed(1)}% confidence)`,
    )
    const alternatives = result.configuration.anchorCandidates
      ?.slice(1)
