...
describe("CLI", () => {
  ...
  it("prints all required JSON data", () => {
    ...
    const options = parseArguments([
      "primary", "#89E5D2", "--seed", "exact", "--format", "json",
    ]);
  ...
  it("prints Tailwind @theme variables", () => {
    const output = vi.spyOn(console, "log").mockImplementation(() => undefined);
    runCli(parseArguments([
      "brand", "#89E5D2", "--seed", "canonical", "--format", "tailwind",
    ]));
  ...
  it("rejects invalid options", () => {
    expect(() => parseArguments(["primary", "#fff", "--seed", "maybe"]))
      .toThrow(/--seed must be one of/);
    expect(() => parseArguments(["primary", "#fff", "--at", "450"]))
      .toThrow(/--at must be auto/);
  });
});
